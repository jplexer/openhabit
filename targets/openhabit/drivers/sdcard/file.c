...
// openhabit: SD-card backend. Reuse all the FAT32 file/dir/getfree code below, but
// mount the card over SDMMC (esp_vfs_fat_sdmmc_mount) instead of internal flash —
// see startFS()/stopFS(). The card is FAT32, so everything else is identical.
#ifndef MODDEF_FILE_SDCARD
    #define MODDEF_FILE_SDCARD 0
#endif
#if MODDEF_FILE_SDCARD
    #undef MODDEF_FILE_FAT32
    #define MODDEF_FILE_FAT32 1
    #include "driver/sdmmc_host.h"
    #ifndef MODDEF_SDCARD_WIDTH
        #define MODDEF_SDCARD_WIDTH 4
    #endif
    #ifndef MODDEF_SDCARD_INTERNAL_PULLUP
        #define MODDEF_SDCARD_INTERNAL_PULLUP 1
    #endif
#endif
...
#if MODDEF_FILE_SDCARD
    if (gCard)          // mounted once for the session — keep it
        return 0;

    sdmmc_host_t host = SDMMC_HOST_DEFAULT();
    host.max_freq_khz = SDMMC_FREQ_DEFAULT;     // conservative bring-up speed

    sdmmc_slot_config_t slot = SDMMC_SLOT_CONFIG_DEFAULT();
    slot.width = MODDEF_SDCARD_WIDTH;
    slot.clk = MODDEF_SDCARD_CLK;
    slot.cmd = MODDEF_SDCARD_CMD;
    slot.d0 = MODDEF_SDCARD_D0;
#if MODDEF_SDCARD_WIDTH == 4
    slot.d1 = MODDEF_SDCARD_D1;
    slot.d2 = MODDEF_SDCARD_D2;
    slot.d3 = MODDEF_SDCARD_D3;
#endif
#if MODDEF_SDCARD_INTERNAL_PULLUP
    slot.flags |= SDMMC_SLOT_FLAG_INTERNAL_PULLUP;  // weak; external 10k recommended
#endif

    esp_vfs_fat_sdmmc_mount_config_t conf = {
        .format_if_mount_failed = false,        // never reformat the user's card
        .max_files = MODDEF_FILE_MAXOPEN,
        .allocation_unit_size = 16 * 1024
    };

    ret = esp_vfs_fat_sdmmc_mount(MODDEF_FILE_ROOT, &host, &slot, &conf, &gCard);
    if (ESP_OK == ret)
        modLog("file: SD card mounted");
#elif MODDEF_FILE_FAT32
...
#if MODDEF_FILE_SDCARD
    // keep the card mounted for the session — the SDMMC mount is slow and the card
    // isn't removable on this board, so we don't unmount when the last file closes.
#elif MODDEF_FILE_FAT32
